  ...
  if(member.permission.has('manageChannels') || member.id){
    if(member.voiceState.channelID != null){
      ...
      if(guildCashe[guild['id']]['linkedChannels']['channels'][voiceChannel]){
        ...
        bot.createMessage(msg.channel.id,"Unable to link this channel. You have already linked the maximum number of channels");
        }
      } else {
      ...
      bot.createMessage(msg.channel.id,"You must be in a voice channel that I can see to link channels");
    }
  }
}
